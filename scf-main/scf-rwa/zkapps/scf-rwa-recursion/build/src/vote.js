import { Field, PrivateKey, SelfProof, ZkProgram, Struct, Bool, Provable, Poseidon, MerkleMap, MerkleTree, MerkleWitness, MerkleMapWitness, } from 'o1js';
...
class VoteState extends Struct({
    ...
    static applyVote(state, voteFor, privateKey, voterWitness, nullifierWitness) {
    ...
    static assertInitialState(state) {
...
const Vote = ZkProgram({
    ...
    methods: {
        ...
        applyVote: {
            ...
            async method(newState, earlierProof, voteFor, voter, voterWitness, nullifierWitness) {
                earlierProof.verify();
                const computedState = VoteState.applyVote(earlierProof.publicInput, voteFor, voter, voterWitness, nullifierWitness);
